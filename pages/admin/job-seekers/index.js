...
export default function JobSeekersList() {
  ...
  // Fetch job seekers when page loads or filters change
  useEffect(() => {
  ...
  async function fetchJobSeekers() {
    try {
      ...
      const res = await fetch(`/api/admin/job-seekers?${queryParams.toString()}`);
  ...
  async function handleStatusUpdate(id, newStatus) {
    try {
      const res = await fetch(`/api/admin/job-seekers?id=${id}`, {
        method: 'PUT',
        headers: {
  ...
  return (
    <>
      <Head>
        <title>Manage Job Seekers - Glodinas Flex Work</title>
        <meta name="description" content="Admin dashboard for managing job seekers" />
      </Head>
              ...
              <div className="px-6 py-12 text-center">
                ...
                <p className="mt-1 text-sm text-gray-500">
                  {filters.status || filters.search
                    ...
                    : 'Job seekers will appear here once they submit their information'}
                </p>
              </div>
            ) : (
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {jobSeekers.map((jobSeeker) => (
                      <tr key={jobSeeker.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm text-gray-900">{jobSeeker.email}</div>
                          <div className="text-sm text-gray-500">{jobSeeker.phone}</div>
                        </td>
                        <td className="px-6 py-4">
                          <div className="text-sm text-gray-900 max-w-xs truncate">{jobSeeker.skills}</div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm text-gray-900">{jobSeeker.preferredLocation}</div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                            jobSeeker.status === 'pending'
                              ? 'bg-yellow-100 text-yellow-800'
                              : jobSeeker.status === 'reviewed'
                              ? 'bg-green-100 text-green-800'
                              : 'bg-blue-100 text-blue-800'
                          }`}>
                            {jobSeeker.status.charAt(0).toUpperCase() + jobSeeker.status.slice(1)}
                          </span>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                          {formatDate(jobSeeker.createdAt)}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                          <div className="flex justify-end space-x-2">
                            {jobSeeker.status === 'pending' && (
                              <button
                                onClick={() => handleStatusUpdate(jobSeeker.id, 'reviewed')}
                            ...
                            {jobSeeker.status === 'reviewed' && (
                              <button
                                onClick={() => handleStatusUpdate(jobSeeker.id, 'matched')}
                                className="text-blue-600 hover:text-blue-900"
                              >
                                Mark Matched
                              </button>
                            )}
                            {jobSeeker.status === 'matched' && (
                              <button
                                onClick={() => handleStatusUpdate(jobSeeker.id, 'reviewed')}
                ...
                <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                  <div>
                    <p className="text-sm text-gray-700">
                      Showing <span className="font-medium">{jobSeekers.length > 0 ? (pagination.page - 1) * pagination.limit + 1 : 0}</span> to{' '}
